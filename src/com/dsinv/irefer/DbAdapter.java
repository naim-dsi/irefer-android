...
public class DbAdapter {
	...
	public static final String tname[] = { "t_users", "t_practice",
			"t_hospital", "t_insurance", "t_specialty", "t_resource",
			"t_doctor", "t_county", "t_doc_fts", "t_doc_report",
			"t_statistics",
			"t_state",// NEW//
			"t_doc_practice", "t_doc_hospital", "t_doc_insurance",
			"t_doc_speciality", "t_doc_plan", "t_doc_aco", "t_plan", "t_aco" };
	// 5324
	public static final String tcols[][] = {
			/* users */{ PK, "user_id", "last_name", "first_name", "email",
					"act_code", "my_prac_id", "my_hos_id", "my_county_id",
					"need_to_sync", "update_setting", "rank_doc",
					"rank_doc_practice", "doc_id" },
	...
	/*
	 * private static final int tcvar[][] = { {}, {6}, {1}, {}, {}, {1,2}, {1},
	 * {1,2,5,6}, {}, {} };
	 * 
	 * private static final int tcref[][] = { {}, {TIMEZONE}, {AIRPORT}, {}, {},
	 * {TIMEZONE, PERSON}, {TRIP}, {TRIP, TRIPDAY, AIRPORT, AIRPORT}, {}, {} };
	 */

	// flightleg = [flight].[src_airport].[dst_airport].[Y].[M].[D].[h].[m] >>>>
	// so unique!!
	...
	public DbAdapter open() throws SQLException {
		if (debug != null)
			dbHelper = new DbHelper(ctx, debug);
		else
	...
	public class StringAndLong extends Pair<String, Long> implements
			Comparable<StringAndLong> {
	...
	public String getFormattedDataSingleRow(int tableId, String format,
			Cursor cur) {
	...
	private String toSQLTimeStamp(String date, String time)
			throws RuntimeException {
		return toSQLDate(date) + " " + toSQLTime(time);
	}

	private ContentValues formatInstance(int tableId, String[] data)
			throws RuntimeException {
	...
	public void populateFTS(long docId, String[] data) {
		...
		try {
			if (!Utils.isEmpty(data[10]))
				text += ("".equals(text) ? "" : ",")
						+ getNameById(PRACTICE, Integer.parseInt(data[10]));
			if (!Utils.isEmpty(data[11]))
				text += ("".equals(text) ? "" : ",")
						+ getNameById(HOSPITAL, Integer.parseInt(data[11]));
			if (!Utils.isEmpty(data[12]))
				text += ("".equals(text) ? "" : ",")
						+ getNameById(SPECIALTY, Integer.parseInt(data[12]));
			if (!Utils.isEmpty(data[13]))
				text += ("".equals(text) ? "" : ",")
						+ getNameById(INSURANCE, Integer.parseInt(data[13]));
			if (!Utils.isEmpty(data[16]))
				text += ("".equals(text) ? "" : ",")
						+ getNameById(COUNTY, Integer.parseInt(data[16]));
		} catch (Exception ex) {
			ex.printStackTrace();
			// continue
		}
		// System.out.println("SMM::FTS::SPEC-ID::"+data[12]+"::SPEC-CODE::"+getNameById(SPECIALTY,
		...
		db.insert(tname[DOC_FTS], null,
				formatInstance(DOC_FTS, new String[] { docId + "", text }));
		// System.out.println("DOC FTS updated for docId="+docId);
	}

	public long insertDoctor(String ftsText, String[] data)
			throws RuntimeException {
		long id = db.insert(tname[DOCTOR], null, formatInstance(DOCTOR, data));
		db.insert(tname[DOC_FTS], null,
				formatInstance(DOC_FTS, new String[] { id + "", ftsText }));
		return id;
	}

	public void insertAllDoctorFTS() {
		db.beginTransaction();
		Cursor cr = dbr.query(tname[DOCTOR], tcols[DOCTOR], null, null, null,
				null, "u_rank, grade, first_name");
		if (cr != null && cr.getCount() > 0) {
			cr.moveToFirst();
			for (int i = 0, j = cr.getCount(); i < j; i++) {
				String ftsText = cr.getString(2).toLowerCase() + ","
						+ cr.getString(3).toLowerCase() + ","
						+ cr.getString(4).toLowerCase();
				try {
					if (!Utils.isEmpty(cr.getInt(11) + ""))
						ftsText += ("".equals(ftsText) ? "" : ",")
								+ getNameById(PRACTICE, cr.getInt(11));
					if (!Utils.isEmpty(cr.getInt(12) + ""))
						ftsText += ("".equals(ftsText) ? "" : ",")
								+ getNameById(HOSPITAL, cr.getInt(12));
					if (!Utils.isEmpty(cr.getInt(13) + ""))
						ftsText += ("".equals(ftsText) ? "" : ",")
								+ getNameById(SPECIALTY, cr.getInt(13));
					if (!Utils.isEmpty(cr.getInt(14) + ""))
						ftsText += ("".equals(ftsText) ? "" : ",")
								+ getNameById(INSURANCE, cr.getInt(14));
					if (!Utils.isEmpty(cr.getInt(16) + ""))
						ftsText += ("".equals(ftsText) ? "" : ",")
								+ getNameById(COUNTY, cr.getInt(16));
				} catch (Exception ex) {
					ex.printStackTrace();
					// continue
				}
				db.insert(
						tname[DOC_FTS],
						null,
						formatInstance(DOC_FTS, new String[] {
								cr.getInt(1) + "", ftsText }));
				cr.moveToNext();
			}
		}
	...
	public void insertDoctor(List ftsArr, List dataArr) throws RuntimeException {
		...
		try {
			...
			for (int i = 0, j = dataArr.size(); i < j; i++) {
				...
				long id = db.insert(tname[DOCTOR], null,
						formatInstance(DOCTOR, data));
				// if(i==0)
				// {
				...
				//
				// for(tblQueryGen = 1 ; tblQueryGen<tcols[DOCTOR].length ;
				// tblQueryGen++)
				// {
				// docSQL.append(", ");
				//
				...
				// }
				// else
				// if(dbHelper.getType(ttypes[DOCTOR].charAt(tblQueryGen))=="TEXT")
				// {
				// docSQL.append("'"+data[tblQueryGen-1]+"' ");
				// }
				// else
				// if(dbHelper.getType(ttypes[DOCTOR].charAt(tblQueryGen))=="DATE")
				// {
				// docSQL.append("'"+data[tblQueryGen-1]+"' ");
				// }
				// else
				// if(dbHelper.getType(ttypes[DOCTOR].charAt(tblQueryGen))=="REAL")
				// {
				...
				// db.insert(tname[DOC_FTS], null, formatInstance(DOC_FTS, new
				// String[]{ id+"", ftsText}));
			}
			// db.beginTransaction();
			// try{
	...
	public void insertDoctorFTS(List ftsArr, List dataArr)
			throws RuntimeException {
		db.beginTransaction();
		for (int i = 0, j = dataArr.size(); i < j; i++) {
			String ftsText = (String) ftsArr.get(i);
	...
	public long getCount(int tableId) {
		// System.out.println("SMM:::::"+DatabaseUtils.longForQuery(db,
		// "SELECT count(DISTINCT doc_id) from t_doctor", null));
		if (tableId == COUNTY) {
			int j = 0;
			Cursor cr = dbr.query(true, tname[tableId], tcols[tableId], null,
					null, tcols[tableId][1], null, "name", null);
			if (cr.getCount() > 0) {
				cr.moveToFirst();
	...
	public int update(int tableId, long rowId, String[] data)
			throws RuntimeException {
		return db.update(tname[tableId], formatInstance(tableId, data),
				matchRow(rowId), null);
	}
	...
	public Cursor fetchAll(int tableId) throws RuntimeException {
		...
		if (tableId == 0) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "last_name", null);
		} else if (tableId == 6) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "u_rank, grade, first_name", null);
		} else if (tableId >= 8) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, null, null);
		} else {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "name", null);
		}
	}

	public Cursor fetchLimited(int tableId) throws RuntimeException {

		if (tableId == 0) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "last_name", "50");
		} else if (tableId == 6) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "u_rank, grade, first_name", "50");
		} else if (tableId >= 8) {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, null, "50");
		} else {
			return dbr.query(true, tname[tableId], tcols[tableId], null, null,
					tcols[tableId][1], null, "name", "50");
		}
	}
	...
	public Cursor fetch(int tableId, long rowId) throws RuntimeException {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				matchRow(rowId), null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public Cursor fetchReportByDocId(long docId) {
		Cursor cur = dbr.query(true, tname[DOC_REPORT], tcols[DOC_REPORT],
				"doc_id = " + docId, null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public Cursor fetchByNetId(int tableId, long netId, String columnName) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId], columnName
				+ "=" + netId, null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public Cursor fetchByNetId(int tableId, long netId) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				tcols[tableId][1] + "=" + netId, null, null, null, null, null);
		if (cur != null){
			cur.moveToFirst();
		}
		else{
	...
	public Cursor fetchInsurance(int tableId, String docins) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				tcols[tableId][1] + " IN (" + docins + ")", null, null, null,
				null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	...
	public Cursor fetchdocInsurance(int tableId, long netId) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				tcols[tableId][2] + "=" + netId, null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public int getDoctorIdbyUserId(int tableId, String userId) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				tcols[tableId][1] + "=" + userId, null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		int s = cur.getInt(13);
	...
	public Cursor fetchByNetIds(int tableId, String netId) {
		Cursor cur = dbr.query(true, tname[tableId], tcols[tableId],
				tcols[tableId][1] + " IN (" + netId + ")", null, null, null,
				null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	...
	public Cursor searchDoctor(String insuIds, String specIds, String hospIds,
			String cntyIds, String docName, String zipCode, String languages,
			String limit, int searchOrder, String acoIds, int resourceFlag,
			String pracIds) {
		/*
		 * System.out.println("SMM::SEARCH-SQL::(last_name LIKE '%"+docName+"%'"+
		 * " OR first_name LIKE '%"
		 * +docName+"%'"+" OR mid_name LIKE '%"+docName+"%' )"+
		...
		if (!Utils.isEmpty(pracIds)) {
			String pracarr[] = pracIds.split(",");

			for (int i = 0, j = pracarr.length; i < j; i++) {
				if (i == 0)
					pracStr = " AND ( prac_ids LIKE '%," + pracarr[i] + ",%'";
				else
					pracStr = pracStr + " OR prac_ids LIKE '%," + pracarr[i]
							+ ",%'";
			}
			if (!Utils.isEmpty(pracStr))
				pracStr = pracStr + ")";
			Log.d("NI::", "DPRACTICES : " + pracIds);
		}
		...
		String qq = "_id IS NOT NULL"
				+ (Utils.isEmpty(docName) ? "" : " AND (last_name LIKE '%"
						+ docName + "%'" + " OR first_name LIKE '%" + docName
						+ "%'" + " OR mid_name LIKE '%" + docName + "%' )")
				...
				+ (Utils.isEmpty(zipCode) ? "" : " AND zip_code = '" + zipCode
						+ "' ")
				+ ((resourceFlag == -1) ? "" : " AND res_flag = '"
						+ resourceFlag + "'")
				+ (Utils.isEmpty(langStr) ? "" : langStr);
		Cursor cur = dbr.query(true, tname[DOCTOR], tcols[DOCTOR], qq, null,
				null, null, orderStr, limit == null ? "50" : limit);

		String q = "SELECT * FROM " + tname[DOCTOR] + " WHERE " + qq
				+ " ORDER BY " + orderStr + " LIMIT " + limit;
	...
	public int countDoctor(String insuIds, String specIds, String hospIds,
			String cntyIds, String docName, String zipCode, String languages,
			String limit) {
		...
		Cursor cur = dbr.query(
				...
				"(last_name LIKE '%"
						...
						+ docName
						+ "%' )"
						+ (Utils.isEmptyNumber(specIds) ? ""
								: "AND spec_id IN (" + specIds + ") ")
						+ (Utils.isEmptyNumber(hospIds) ? ""
								: "AND hosp_id IN (" + hospIds + ") ")
						+ (Utils.isEmptyNumber(insuIds) ? ""
								: "AND insu_id IN (" + insuIds + ") ")
						+ (Utils.isEmpty(zipCode) ? "" : "AND zip_code = '"
								+ zipCode + "')")
						+ (Utils.isEmpty(langStr) ? "" : langStr), null, null,
				null, "u_rank DESC, grade DESC, first_name", null);
	...
	public Cursor fetchPracticeByCode(String code) throws RuntimeException {
		Cursor cur = dbr.query(true, tname[PRACTICE], tcols[PRACTICE],
				"name LIKE '%" + code + "%'", null, null, null, null, null);
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public Cursor getMyPractice() throws RuntimeException {
		...
		if (cur != null) {
			...
			cur.close();
			cur1 = dbr.query(true, tname[this.PRACTICE], tcols[this.PRACTICE],
					"prac_id = " + pracId, null, null, null, null, null);
	...
	public Cursor fetchDoctorByName(String code) throws RuntimeException {
		Cursor cur = dbr.query(true, tname[DOCTOR], tcols[DOCTOR],
				"last_name LIKE '%" + code + "%' OR first_name LIKE '%" + code
						+ "%' OR mid_name LIKE '%" + code + "%'", null, null,
				null, "u_rank DESC, grade DESC, first_name", "50");
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	public Cursor fetchDoctorFTS2(String code) throws RuntimeException {
		...
		for (int i = 0, j = arr.length; i < j; i++) {
			str += ("".equals(str) ? " WHERE " : " AND ")
					+ "LOWER(doc.last_name) LIKE ? ";
			arg[i] = "%" + arr[i].toLowerCase() + "%";
		}
		// String arr2[] = new String[]{"doc._id","doc.doc_id","doc.first_name",
		// "doc.last_name",
		// "doc.mid_name", "doc.degree", "doc.doc_phone", "doc.language",
		// "doc.grade","doc.u_rank"};
	...
	public Cursor fetchDoctorFTS(String code) throws RuntimeException {
		...
		Cursor cur0 = dbr.query(true, tname[DOC_FTS],
				new String[] { "doc_id" }, "text MATCH '" + key + "'", null,
				null, null, null, "50");
		String ids = "";
		if (cur0 != null) {
			cur0.moveToFirst();
		...
		Cursor cur = dbr.query(true, tname[DOCTOR], tcols[DOCTOR],
				"doc_id IN (" + ids + ")", null, null, null,
				"u_rank DESC, grade DESC, first_name", "50");
		if (cur != null)
			cur.moveToFirst();
		return cur;
	}
	...
	/*
	 * //returns CURSOR or NULL public Cursor fetchFramingByQuestion(int secId,
	 * int qesIdx) throws RuntimeException { Cursor cur = dbr.query(true,
	 * tname[OPENING_FRAMING], tcols[OPENING_FRAMING],
	 * "sec_id="+secId+" AND qes_idx="+qesIdx, null, null, null, null, null);
	 * if(cur!=null) cur.moveToFirst(); return cur; }
	 * 
	 * //returns CURSOR or NULL public Cursor fetchDimensionByFraming(int
	 * framingId) throws RuntimeException { Cursor cur = dbr.query(true,
	 * tname[OPENING_DIMENSION], tcols[OPENING_DIMENSION],
	 * "frameing_id="+framingId, null, null, null, null, null); if(cur!=null)
	 * cur.moveToFirst(); return cur; } public Cursor fetchDimensionById(int id)
	 * throws RuntimeException { Cursor cur = dbr.query(true,
	 * tname[OPENING_DIMENSION], tcols[OPENING_DIMENSION], "_id="+id, null,
	 * null, null, null, null); if(cur!=null) cur.moveToFirst(); return cur; }
	 */
	// returns STRING or NULL
	public String fetchFormattedDataSingleRow(int tableId, long rowId,
	...
	private static class DbHelper extends SQLiteOpenHelper {
		...
		private String createTableCommand(int id) {
			if (id == DOC_FTS) {
				return "CREATE VIRTUAL TABLE " + tname[DOC_FTS]
						+ " using FTS3(doc_id, text)";
			}
			...
			/*
			 * for(int i=0;i<tcvar[id].length;i++){ s.append(", CONSTRAINT ");
		...
		public void onCreate(SQLiteDatabase db) throws RuntimeException {
			 ...
			 * for(int i=0;i<tname.length;i++){ tcolmap[i].clear(); for(int
			 * j=0;j<tcols[i].length;j++) tcolmap[i].put(tcols[i][j], j); }
			 */
		}
