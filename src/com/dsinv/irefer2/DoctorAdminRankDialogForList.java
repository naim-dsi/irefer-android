...
public class DoctorAdminRankDialogForList extends Dialog {
	...
	public DoctorAdminRankDialogForList(final DoctorListActivity ctx, int style, 
			final DbAdapter dba,
			final int isOnlineSearch, final int position){
		super(ctx, style);
		setContentView(R.layout.doctor_admin_rankdialog);
		setCancelable(true);
		final HashMap<String, String> data = ctx.docList.get(position);
		//there are a lot of settings, for dialog, check them all out!
		int userRankValue = Integer.parseInt((String)data.get("u_rank"));
		int adminRankValue = Integer.parseInt((String)data.get("up_rank"));
