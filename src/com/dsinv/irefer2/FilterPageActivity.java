...
public class FilterPageActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// findViewById(R.id.setup_bottom_wrapper1);
		// LinearLayout l2 = (LinearLayout)
		// findViewById(R.id.setup_bottom_wrapper2);
		// try{
		...
		// new Integer(windowWidth/2),
		// android.view.ViewGroup.LayoutParams.WRAP_CONTENT));
		// //l1.setLayoutParams(new LinearLayout.LayoutParams(new
		// Integer(windowWidth/2), LinearLayout.LayoutParams.FILL_PARENT));
		// //l2.setLayoutParams(new LinearLayout.LayoutParams(new
		// Integer(windowWidth/2), LinearLayout.LayoutParams.FILL_PARENT));
		// }
		// catch(Exception ex)
		// {
		...
		/*
		 * Cursor cr1 = dba.getMyPractice(); if(cr1 != null && cr1.getCount() >
		 * 0) { headline1.setText(cr1.getString(2));
		 * headline2.setText(cr1.getString(3)); }
		 */
		...
		insuranceBtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent(FilterPageActivity.this,
						ItemSelectActivity.class);
				;

				// if( onlineSearch.isChecked() ) {
				if (onlineFlag == 1) {
					// System.out.println("online select activity ...");
					intent = new Intent(FilterPageActivity.this,
							ItemOnlineSelectActivity.class);
				}
				...
				// startActivityForResult(intent, 101);
				// AlertDialog.Builder builder =
				// getDialogBuilder(DbAdapter.INSURANCE);
				// builder.show();
			}
		});
		...
		specialityBtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				// AlertDialog.Builder builder =
				// getDialogBuilder(DbAdapter.SPECIALTY);
				// builder.show();
			}
		});
		...
		acoBtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				// else {
				// AlertDialog.Builder builder =
				// getDialogBuilder(DbAdapter.ACO);
				// builder.show();
				// }
			}
		});
		...
		getDocBtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				/*
				 * if("None".equals(insuranceValue.getText()) ||
				 * insuranceValue.getText() == null ||
				 * "".equals(insuranceValue.getText())) {
				 * Toast.makeText(FilterPageActivity.this,
				 * "Please select Insurance.", Toast.LENGTH_SHORT).show();
				 * return; } if("None".equals(specialityValue.getText()) ||
				 * specialityValue.getText() == null ||
				 * "".equals(specialityValue.getText())) {
				 * Toast.makeText(FilterPageActivity.this,
				 * "Please select Speciality.", Toast.LENGTH_SHORT).show();
				 * return; }
				 */
				...
				if (onlineFlag == 1) {
					...
					// intent = new Intent(FilterPageActivity.this,
					// DoctorOnlineListActivity.class);
				}
				if (resourceFlag == 1) {
		 ...
		 * setupBtn.setOnClickListener(new View.OnClickListener() { public void
		 * onClick(View v) { if(fromPageId == Utils.SETUP_PAGE) { finish(); }
		 * else { Intent intent = new Intent(FilterPageActivity.this,
		 * SetupActivity.class); intent.putExtra(Utils.pageId,
		 * Utils.FILTER_PAGE); startActivityForResult(intent, 1100); } } });
		 */
		Button advBtn = (Button) findViewById(R.id.filter_bottom_button2);
		advBtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent(FilterPageActivity.this,
						DoctorListAdvanceActivity.class);
				// faisal > starts
				if (onlineFlag == 1) {
					// intent = new Intent(FilterPageActivity.this,
					// DoctorOnlineListActivity.class);
		...
		/*
		 * dba = new DbAdapter(this); dba.open();
		 * 
		 * Cursor cr = dba.fetchAll(dba.USERS); if(cr != null && cr.getCount() >
		 * 0) { cr.moveToFirst(); userName = "User: "+cr.getString(3); userName
		 * = userName + " " +cr.getString(2); userEmail = "Email: "
		 * +cr.getString(4); pracId = cr.getInt(6); cr.close(); reg = 1;
		 * System.out.println("USER_NAME="+userName); }
		 * 
		 * if(pracId > 0) { Cursor cr1 = dba.fetchAll(dba.PRACTICE); if(cr1 !=
		 * null && cr1.getCount() > 0) { cr1.moveToFirst(); pracName =
		 * cr1.getString(2); pracAddress = cr1.getString(3); cr1.close(); } }
		 */
		// insuranceValue= (TextView) findViewById(R.id.headLabel1);
		if (myPracId == 0 || myPracId < 0) // ha ha ha :: what a mistake
											// if(myPracId <= 0 )
			loadPractice();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		String[] resultIdArr = (String[]) data
				.getStringArrayExtra("selectedIdArr");
		...
		if (requestCode == 101) {
			...
			for (int i = 0; i < arr.length; i++) {
				...
				position = find(insuranceIdArr,
						Integer.parseInt(resultIdArr[i]));
				if (position != -1) {
	...
	private AlertDialog.Builder getDialogBuilder(final int opt) {
		...
		case DbAdapter.HOSPITAL:
			builder.setMultiChoiceItems(hospitalNameArr, hospitalSelection,
						...
						public void onClick(DialogInterface dialog, int which,
							...
							for (int i = 0; i < hospitalSelection.length; i++) {
								if (hospitalSelection[i]) {
									...
									hospitalName = hospitalName
											+ hospitalNameArr[i].toString();
									hospitalId = hospitalId
											+ new Integer(hospitalIdArr[i])
													.toString();
								}
							}
		...
		case DbAdapter.INSURANCE:
			builder.setMultiChoiceItems(insuranceNameArr, insuranceSelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
							...
							for (int i = 0; i < insuranceSelection.length; i++) {
								if (insuranceSelection[i]) {
									...
									insuranceName = insuranceName
											+ insuranceNameArr[i].toString();
									insuranceId = insuranceId
											+ new Integer(insuranceIdArr[i])
													.toString();
								}
							}
		...
		case DbAdapter.SPECIALTY:
			builder.setMultiChoiceItems(specialityNameArr, specialitySelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
							...
							for (int i = 0; i < specialitySelection.length; i++) {
								if (specialitySelection[i]) {
									...
									specialityName = specialityName
											+ specialityNameArr[i].toString();
									specialityId = specialityId
											+ new Integer(specialityIdArr[i])
													.toString();
								}
							}
			...
			break;
		case DbAdapter.COUNTY:
			builder.setMultiChoiceItems(countyNameArr, countySelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
							...
							if (isChecked) {
								...
								if (totalCounty.length < Utils.COUNTY_LIMIT) {
									...
									for (int i = 0; i < countySelection.length; i++) {
										if (countySelection[i]) {
											if (!countyName.equals("All")) {
												...
												countyName = countyNameArr[i]
														.toString();
												countyId = new Integer(
														countyIdArr[i])
														.toString();
											}
										}
									...
									Toast.makeText(
											FilterPageActivity.this,
											"Maximum " + Utils.COUNTY_LIMIT
													+ " county can be seleted",
			...
			break;
		case DbAdapter.LANGUAGE:
			builder.setMultiChoiceItems(languageArr, languageSelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
								...
								if (languageSelection[i]) {
									if (!languageName.equals("All")) {
										languageName = languageName + ","
												+ languageArr[i].toString();
										languageId = languageId
												+ ","
												+ new Integer(languageIdArr[i])
														.toString();
									} else {
										languageName = languageArr[i]
												.toString();
										languageId = new Integer(
												languageIdArr[i]).toString();
									}
								}
							}
			...
			break;
		case DbAdapter.ACO:
			builder.setMultiChoiceItems(acoArr, acoSelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
							...
							for (int i = 0; i < acoSelection.length; i++) {
								if (acoSelection[i]) {
									if (!acoName.equals("All")) {
										...
										acoId = acoId
												+ ","
												+ new Integer(acoIdArr[i])
														.toString();
									} else {
										acoName = acoArr[i].toString();
										acoId = new Integer(acoIdArr[i])
												.toString();
									}
								}
							}
			...
			break;
		case DbAdapter.OFFICE_HOUR:
			builder.setMultiChoiceItems(officeHourArr, officeHourSelection,
					new DialogInterface.OnMultiChoiceClickListener() {
						public void onClick(DialogInterface dialog, int which,
								boolean isChecked) {
							// TODO Auto-generated method stub
							...
							for (int i = 0; i < officeHourSelection.length; i++) {
								if (officeHourSelection[i]) {
									if (!officeHourName.equals("All")) {
										officeHourName = officeHourName + ","
												+ officeHourArr[i].toString();
										...
										officeHourName = officeHourArr[i]
												.toString();
										officeHourId = new Integer(
												officeHourIdArr[i]).toString();
									}
								}
							}
